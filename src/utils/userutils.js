...
async function initiateSaleUtil(ownershipId, buyerAadhar) {
  ...
  const [buyer, contractRes] = await Promise.all([
    buyerPromise,
    contractPromise,
  ]);
  ...
  try {
    ...
    return {
      success: false,
      reason: `Error in initiating sale : ${errorToMessageMap[processedReason]}`,
    };
  }
  fetchUtil.post(`${baseUrl}/ownerships/${ownershipId}/action/SALE_INITIATED`, {
    body: { buyerId: buyer._id, transactionHash: result.hash },
  });
  return { success: true };
}
...
async function approveDocumentsUtil(ownershipId) {
  ...
  try {
    ...
    return {
      success: false,
      reason: `Error in approving documents. ${errorToMessageMap[processedReason]}`,
    };
  }
  fetchUtil.post(`${baseUrl}/ownerships/${ownershipId}/action/DOC_APPROVED`, {
    body: { transactionHash: result.hash },
  });
  return { success: true };
}
...
async function rejectDocumentsUtil(ownershipId) {
  ...
  try {
    ...
    return {
      success: false,
      reason: `Error in rejecting documents. ${errorToMessageMap[processedReason]}`,
    };
  }
  fetchUtil.post(`${baseUrl}/ownerships/${ownershipId}/action/DOC_REJECT`, {
    body: { transactionHash: result.hash },
  });
  return { success: true };
}
...
async function initiatePaymentUtil(ownershipId) {
  ...
  try {
    ...
    return {
      success: false,
      reason: `Error in initiating payments. ${errorToMessageMap[processedReason]}`,
    };
  }
  fetchUtil.post(`${baseUrl}/ownerships/${ownershipId}/action/TX_INITIATED`, {
    body: { transactionHash: result.hash },
  });
  return { success: true };
}
...
async function closeSaleUtil(ownershipId) {
  ...
  const [ownership] = await Promise.all([ownershipPromise]);
  const buyer = await getUserInfoWithId(ownership.buyerId);
  let result;
  try {
    const hashArray = _.map(ownership.property.pixels, (pixel) => pixel.hash);
    result = await approveDocumentsAndGetId(
      hashArray,
      buyer.walletAddress,
      contract
    );
    ...
    return {
      success: false,
      reason: `Error in closing sale. ${errorToMessageMap[processedReason]}`,
    };
  }
  fetchUtil.post(`${baseUrl}/ownerships/${ownershipId}/action/CLOSED`, {
    body: { transactionHash: result.hash, newOwnershipId: result.ownershipId },
  });
}
...
async function cancelSaleUtil(ownershipId) {
  ...
  try {
    ...
    return {
      success: false,
      reason: `Error in cancelling sales. ${errorToMessageMap[processedReason]}`,
    };
  }
  fetchUtil.post(`${baseUrl}/ownerships/${ownershipId}/action/CANCEL_SALE`, {
    body: { transactionHash: result.hash },
  });
  return { success: true };
}
